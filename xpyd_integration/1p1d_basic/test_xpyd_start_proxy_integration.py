...
def _launch_proxy_via_script(
    ...
    command = [
        "bash",
        str(_SCRIPT),
        "-pn", str(len(prefill_ports)),
        "-pt", "8",
        "-pd", str(len(prefill_ports)),
        "-pw", "8",
        "-dn", str(len(decode_ports)),
        "-dt", "8",
        "-dd", str(len(decode_ports)),
        "-dw", "8",
        "--prefill-base-port", str(prefill_ports[0]),
        "--decode-base-port", str(decode_ports[0]),
    ]
    return subprocess.Popen(
        command,
        cwd=_PROXY_REPO / "xpyd",
        env=env,
